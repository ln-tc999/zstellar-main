...
description: Integrate the Nethermind Stellar Private Payments (Privacy Pools) PoC into zStellar — pool/verifier/ASP contracts, the UTXO note model, commitments, nullifiers, and the deposit/transfer/withdraw operations. Use when wiring zStellar to on-chain contracts or modeling notes.
---

# Stellar Private Payments PoC — integration

zStellar builds **on top of** this PoC. Do not reinvent the cryptography or contracts; reuse them.
...
## Before building
1. Pin a PoC commit. APIs may drift; record the commit hash in zStellar's README.
...
- Loss of keys/notes = loss of funds → zStellar must support export/import + local backup.
...
- **Transact** — advanced combined op. Out of scope for zStellar MVP UI.
