...
description: Understand on-chain ZK verification primitives on Stellar/Soroban — BN254 pairing host functions, Poseidon/Poseidon2 hashing, and Groth16 proof verification — as used by the zStellar verifier contract. Use when encoding proofs for the verifier, debugging verification failures, or reasoning about hashing.
---

# Stellar ZK primitives (BN254 / Poseidon / Groth16)

Background for how zStellar's on-chain Verifier checks proofs. zStellar does not author the verifier (PoC provides it), but the frontend must encode proofs/public inputs to match.
...
## What zStellar's frontend must get right
